...
export default function ExpensesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/expenses?page=1&limit=100${qs}`)
      .then((r) => r.json())
      .then((data) => {
        const all: ExpenseRow[] = Array.isArray(data) ? data : data.items || []
        const months = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
        const buckets = months.map((m) => ({ month: m, amount: 0 }))
        all.forEach((e) => {
          const d = new Date(e.date)
          if (d.getFullYear() === selectedYear) buckets[d.getMonth()].amount += e.amount
        })
    ...
    <div className="space-y-8">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-600">Manage and track all your expenses in one place</p>
        </div>
        <a href="/dashboard/expenses/create" className="inline-flex items-center justify-center rounded-2xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white shadow hover:bg-slate-800">
          + Create Expense
        </a>
      </header>
            ...
            return (
              <div key={d.month} className="flex-1 flex flex-col items-center gap-2">
                <div className="w-full flex justify-center" style={{ height: '180px' }}>
                  <div className="w-full max-w-10 rounded-t-xl bg-gradient-to-t from-rose-700 to-rose-500 flex items-end justify-center pb-2" style={{ height: `${h}%`, minHeight: d.amount ? '24px' : '4px' }}>
