...
export async function POST(request: NextRequest) {
  ...
  // In prod: fetch webhook URL, log to AuditLog
  const { writeAuditLog } = await import("@/lib/audit");
  // Find userId from invoice
  await writeAuditLog({ userId: (invoice as { userId: string }).userId, action: "webhook", entity: "Invoice", entityId: invoiceId, diff: { event }, ip: request.headers.get("x-forwarded-for") });
  return NextResponse.json({ message: "Webhook received (placeholder)", event, invoiceId });
}
